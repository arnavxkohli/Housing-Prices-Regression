...
def xavier_init(size, gain=1.0):
    """
    Xavier's initialisation of network weights.
...
class SigmoidLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        result = np.multiply(result, (1 - result))
...
class IdentityLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        Given `grad_z`, the gradient of some scalar (e.g. loss) with respect to
        the output of this layer, performs back pass through the layer (i.e.
        computes gradients of loss with respect to parameters of the layer and
        inputs of the layer).
...
class LinearLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        self._grad_b_current = np.mean(grad_z, axis=0)
    ...
    def update_params(self, learning_rate):
        ...
        self._b -= learning_rate * self._grad_b_current.reshape(*self._b.shape)
...
class MultiLayerNetwork(object):
    ...
    def __init__(self, input_dim, neurons, activations):
        ...
        self._layers = []
        for neuron_count, activation in zip(neurons, activations):
            linear_layer = LinearLayer(input_dim, neuron_count)
            ...
            if activation == 'sigmoid':
                ...
                activation_function = IdentityLayer()
            else:
...
class Trainer(object):
    ...
    def __init__(
        ...
        # Added pre-processing to trainer. Test case 3 on labts does not
        # implement pre-processing. This is a redundant operation if pre-processing
        # has already been implemented outside of the training process.
        self.prep_input = None
    ...
    def train(self, input_dataset, target_dataset):
        ...
        self.prep_input = Preprocessor(input_dataset)
        input_dataset = self.prep_input.apply(input_dataset)

        for _ in range(self.nb_epoch):
            if self.shuffle_flag:
                input_dataset, target_dataset = self.shuffle(input_dataset, target_dataset)

            for input_batch, target_batch in zip(
                    np.array_split(input_dataset, len(input_dataset) // self.batch_size),
                    np.array_split(target_dataset, len(target_dataset) // self.batch_size)):
                # Forward pass
                outputs = self.network.forward(input_batch)
                # Compute loss
                self._loss_layer.forward(outputs, target_batch)
                # Backward pass
                gradient_loss_wrt_outputs = self._loss_layer.backward()
                self.network.backward(gradient_loss_wrt_outputs)
...
class Preprocessor(object):
    ...
    def __init__(self, data):
        ...
        Arguments:
            ...
            the normalization.
        """
        self.min_data = np.minimum.reduce(data, axis=0)
        self.max_data = np.maximum.reduce(data, axis=0)
    ...
    def revert(self, data):
        ...
        """
        return data * np.tile(self.max_data - self.min_data, (data.shape[0], 1)) + self.min_data
